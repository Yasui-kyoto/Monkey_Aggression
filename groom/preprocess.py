...
class DataCleaner:
    ...
    def plot_behavior_scatter(self, df, y_column, x_column='delta_time', hue_column='behavior'):
        ...
        # --- 修正ポイント2: タイトルに総数を表示 ---
        total_n = len(df)
        plt.title(f'Time Series Analysis: Scatter plot of {position_dict[y_column]} Change (Total n={total_n})')
    ...
    def plot_highlight_behavior(self, df, target_behavior, y_column, x_column='delta_time'):
        ...
        # --- 修正ポイント2: タイトルに総数を表示 ---
        plt.title(f'Highlight: {target_behavior} ({position_dict[y_column]}) | Total n={total_n}')
